...
namespace NH.JewelryErpMini.Helpers
{
    public class ExcelHelper
    {
        ...
        public static void MyExport(DataTable dtSource, string strHeaderText, string strFileName, string strSheetName, string[] oldColumnNames, string[] newColumnNames)
        {
            ...
            ExcelStream getms = new ExcelStream();

            #region 为getms赋值
            if (oldColumnNames.Length != newColumnNames.Length)
            {
                //getms= new MemoryStream();
                getms = new ExcelStream();
            }
            //excel2003
            ...
            #region 右击文件 属性信息
            //excel2007 没DocumentSummaryInformation与SummaryInformation所以去掉
            //{
            ...
            //    getms= ms;
            //}
            /*上文提到使用NPOI导出excel2007的时候会报错【不能操作已经关闭的流】，原因是在write的时候给Dispose了，故可以重写MemenryStream的Dispose接口*/
            /*　
            ExcelStream stream = new ExcelStream();
            workbook.Write(stream);
            stream.CanDispose = true;
            */
            using (ExcelStream stream = new ExcelStream())
            {
                workbook.Write(stream);
                stream.Flush();
                stream.Position = 0;
        ...
        public static DataTable ImportExcelFile(string filePath)
        {
            ...
            #endregion
            ISheet sheet = null;
            if (hssfworkbook.GetType() == typeof(HSSFWorkbook))
            {
                //excel2003或2003以下
                sheet = ((HSSFWorkbook)hssfworkbook).GetSheetAt(0);
            }
            else if (hssfworkbook.GetType() == typeof(XSSFWorkbook))
            {
                //excel2007或2007以上
                sheet = ((XSSFWorkbook)hssfworkbook).GetSheetAt(0);
            }
            ...
            //handling header.去除标题名称中的所有空格,英文字体全转成小写
            for (int i = headerRow.FirstCellNum; i < cellCount; i++)
            {
                DataColumn column = new DataColumn(headerRow.GetCell(i).StringCellValue.Replace(" ", "").ToLower());
                table.Columns.Add(column);
            }
